...
class TransformerGenerator(object):
  ...
  def __init__(self, transformer_class: Type[Transformer], **kwargs):
  ...
  def create_transformer(self, dataset: Dataset) -> Transformer:
...
transformers = {
    'balancing':
    TransformerGenerator(BalancingTransformer),
}
...
class _MolnetLoader(object):
  ...
  def __init__(self, featurizer: str,
               splitter: str,
               transformer_generators: List[str],
  ...
  def load_dataset(
    ...
    if reload:
      if self.splitter is None:
        if os.path.exists(save_folder):
          transformers = load_transformers(save_folder)
          return self.tasks, (DiskDataset(save_folder),), transformers
      else:
        loaded, all_dataset, transformers = load_dataset_from_disk(
            save_folder)
    ...
    if reload and isinstance(train, DiskDataset) and isinstance(
      ...
      save_dataset_to_disk(save_folder, train, valid, test,
                                               transformers)
